...
bot.on('message', (ctx) => {
  ...
  if (!ctx.message.text || !ctx.message.text.startsWith('/')) return `Please try with /`;   // do nothing if non-text message is sent.
  ...
  if (!command) return `No command`;   // if no command found, return.
